...
// Function to create default tax rate options if they don't exist
function create_default_tax_rates() {
    $provinces = array('AB', 'BC', 'MB', 'NB', 'NL', 'NS', 'ON', 'PE', 'QC', 'SK', 'NT', 'NU', 'YT');
    $default_rates = array(
        'AB' => 5,   // Alberta
        ...
        'NU' => 5,   // Nunavut
        'YT' => 5    // Yukon Territory
    );
    
    foreach ($provinces as $province) {
        $option_name = 'wc_tax_rate_' . $province;
        if (!get_option($option_name)) {
            $rate = isset($default_rates[$province]) ? $default_rates[$province] : 15;
            add_option($option_name, $rate);
            error_log("Tax Rate Debug: Created default tax rate for {$province}: {$rate}%");
        }
    }
}

// Create default tax rates on plugin activation
register_activation_hook(__FILE__, 'create_default_tax_rates');

// Also create tax rates on init (for immediate use)
add_action('init', 'create_default_tax_rates');
...
function d_add_province_class($classes) {
    if (!isset($_GET['province'])) {
        ...
        $classes[] = 'has-province';
        $classes[] = 'province-' . $province;
    }
    return $classes;
}
...
function age_verification_dialog() {
    ...
    // Check if the province parameter exists
...
function get_curent_tax_province($product){
    ...
    $customer_zone = isset($_GET['province']) ? sanitize_text_field($_GET['province']) : null;
